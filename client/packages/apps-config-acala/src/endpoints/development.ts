...
export function createCustom (t: TFunction): LinkOption[] {
  ...
  return WS_URL
    ? [
      {
        isHeader: true,
        text: t('rpc.dev.custom', 'Custom environment', { ns: 'apps-config-acala' }),
        textBy: '',
        value: ''
      },
      {
        info: 'WS_URL',
        text: t('rpc.dev.custom.entry', 'Custom {{WS_URL}}', { ns: 'apps-config-acala', replace: { WS_URL } }),
        textBy: WS_URL,
        value: WS_URL
      }
    ]
  ...
  try {
    ...
    if (storedItems) {
      ...
      return items.map((textBy) => ({
        info: 'local',
        text: t('rpc.dev.custom.own', 'Custom', { ns: 'apps-config-acala' }),
        textBy,
        value: textBy
      }));
    }
  ...
  return [
    {
      dnslink: 'local',
      info: 'local',
      text: t('rpc.dev.local', 'Local Node', { ns: 'apps-config-acala' }),
